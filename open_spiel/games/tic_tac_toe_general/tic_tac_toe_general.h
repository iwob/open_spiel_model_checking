...
#ifndef OPEN_SPIEL_GAMES_TIC_TAC_TOE_GENERAL_H_
#define OPEN_SPIEL_GAMES_TIC_TAC_TOE_GENERAL_H_
...
class TicTacToeGeneralState : public State {
 public:
  TicTacToeGeneralState(std::shared_ptr<const Game> game);

  TicTacToeGeneralState(const TicTacToeGeneralState&) = default;
  TicTacToeGeneralState& operator=(const TicTacToeGeneralState&) = default;
...
// Game object.
class TicTacToeGeneralGame : public Game {
 public:
  explicit TicTacToeGeneralGame(const GameParameters& params);
  int NumDistinctActions() const override { return kNumCells; }
  std::unique_ptr<State> NewInitialState() const override {
    return std::unique_ptr<State>(new TicTacToeGeneralState(shared_from_this()));
  }
...
}  // namespace tic_tac_toetic_tac_toe_general
}  // namespace open_spiel
